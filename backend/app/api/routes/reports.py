...
async def expense_report(
    ...
    group_by: Literal["date", "category"] = "date",
    db: AsyncSession = Depends(deps.get_db),
):
    spent_date = cast(Expense.spent_at, Date)
    if group_by == "date":
        key_col = spent_date
        label_expr = spent_date
    else:
        key_col = Expense.category_id
        label_expr = ExpenseCategory.name_en
    ...
    if group_by == "category":
        stmt = stmt.join(ExpenseCategory, ExpenseCategory.id == Expense.category_id)
