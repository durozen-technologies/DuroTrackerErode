...
export async function exportPurchasesPdf({ dateFrom, dateTo, groupBy, data }: { dateFrom?: string; dateTo?: string; groupBy?: string; data: any }) {
      ...
      <body>
        <div class="header">
          ...
          <div class="meta">
            ...
            ${groupBy ? `<div>Grouped By: <strong style="text-transform: capitalize;">${escapeHtml(groupBy)}</strong></div>` : ''}
  ...
  `;
  await generateAndSharePdf(html, 'Purchases');
}
