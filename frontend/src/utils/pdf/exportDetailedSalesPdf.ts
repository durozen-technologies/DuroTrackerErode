...
export async function exportDetailedSalesPdf({ dateFrom, dateTo, partyName, data }: { dateFrom?: string; dateTo?: string; partyName: string; data: any[] }) {
  if (!data || data.length === 0) {
    Alert.alert('Empty Report', 'There is currently no detailed sales data to export.');
    return;
  }
      ...
      <body>
        <div class="header">
          ...
          <div class="meta">
            ...
            <div>Customer: <strong style="text-transform: capitalize;">${escapeHtml(partyName)}</strong></div>
  ...
  `;
  await generateAndSharePdf(html, 'Sales_Report');
}
