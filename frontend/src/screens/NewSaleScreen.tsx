    ...
    <SafeAreaView className="flex-1 bg-canvas">
      <View className="px-4 py-3 bg-surface border-b border-border flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <Text className="text-lg font-bold text-content-primary">{editData ? 'Edit Sale' : 'New Sale'}</Text>
        </View>
        {editData && (
          <TouchableOpacity
            onPress={() =>
      ...
      >
        ...
        <Text className="text-xs font-medium text-content-secondary mb-1">Customer *</Text>
        ...
        {lines.map((line, idx) => {
          ...
          return (
            <View key={idx} className="bg-surface border border-border rounded-xl p-3 mb-3">
              ...
              {item && (
                <Text className="text-xs text-brand mb-2">
                  Available stock: {item.available_stock} {item.unit_type}
                </Text>
              )}
              <View className="flex-row gap-2">
                <View className="flex-1">
                  <Text className="text-[10px] text-content-tertiary mb-1">{isKg ? 'Kg' : 'Units'}</Text>
                ...
                {isKg && (
                  <View className="flex-1">
                    <Text className="text-[10px] text-content-tertiary mb-1">Count</Text>
                    <TextInput placeholderTextColor="#849CA5"
                ...
                <View className="flex-1">
                  <Text className="text-[10px] text-content-tertiary mb-1">Selling Rate</Text>
                  <TextInput placeholderTextColor="#849CA5"
                    keyboardType="decimal-pad"
                    value={line.rate}
        ...
        <View className="bg-surface border border-border rounded-xl p-4 mb-3">
          ...
          <Text className="text-xs text-content-tertiary mb-1">Cash Received</Text>
          <TextInput placeholderTextColor="#849CA5"
            keyboardType="decimal-pad"
            value={cash}
          ...
          />
          <Text className="text-xs text-content-tertiary mb-1">UPI Received</Text>
          <TextInput placeholderTextColor="#849CA5"
            keyboardType="decimal-pad"
            value={upi}
            onChangeText={setUpi}
          ...
          <View className="flex-row justify-between mb-1">
            <Text className="text-content-secondary">Bill Amount</Text>
            <Text className="font-bold">₹{totalAmount.toLocaleString()}</Text>
          </View>
          <View className="flex-row justify-between mb-1">
            <Text className="text-content-secondary">Received</Text>
            <Text className="font-bold">₹{totalPaid.toLocaleString()}</Text>
          </View>
          <View className="flex-row justify-between">
            <Text className="text-content-secondary">Customer Balance</Text>
